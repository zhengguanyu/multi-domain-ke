...
def compute_z(
    ...
    if torch.isnan(target).any() or torch.isinf(target).any():
        print("WARNING: NaN/inf detected in target, falling back to target_init")
        if torch.isnan(target_init).any() or torch.isinf(target_init).any():
            print("WARNING: target_init is also corrupted, returning zeros")
            return torch.zeros_like(target_init)
        return target_init
