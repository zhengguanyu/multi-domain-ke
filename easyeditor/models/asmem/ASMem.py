...
class ASMem(torch.nn.Module):
    def __init__(self, config, model, device, router):
        super(ASMem, self).__init__()
        self.config = config
        self.model = model
        ...
        if type(adapter_layer) is not ASMemAdapter:

            setattr(self.edit_module, self.layer_name, ASMemAdapter(config, adapter_layer, router=router, transpose=transpose))
    ...
    def get_adapter_layer(self):
        adapter_layer = getattr(self.edit_module, self.layer_name)
        assert type(adapter_layer) is ASMemAdapter, print('Adapter Layer is not added correctly....')
...
class ASMemAdapter(torch.nn.Module):
    def __init__(self, config, layer, transpose, router: KnowRouter):
        super(ASMemAdapter, self).__init__()
